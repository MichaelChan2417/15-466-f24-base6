...
Player *Game::spawn_player() {
  ...
  player.position.x = glm::mix(ArenaMin.x + 2.0f * PlayerRadius,
                               ArenaMax.x - 2.0f * PlayerRadius,
                               0.4f + 0.2f * mt() / float(mt.max()));
  player.position.y = glm::mix(ArenaMin.y + 2.0f * PlayerRadius,
                               ArenaMax.y - 2.0f * PlayerRadius,
                               0.4f + 0.2f * mt() / float(mt.max()));
...
void Game::update(float elapsed) {
  ...
  // check if any player has reached the credit point:
  for (auto &p : players) {
    if (glm::distance2(p.position, credit_point) <
        PlayerRadius * PlayerRadius) {
      p.credit += 1;
      // update random credit point:
      credit_point.x = glm::mix(ArenaMin.x + 2.0f * PlayerRadius,
                                ArenaMax.x - 2.0f * PlayerRadius,
                                0.4f + 0.2f * mt() / float(mt.max()));
      credit_point.y = glm::mix(ArenaMin.y + 2.0f * PlayerRadius,
                                ArenaMax.y - 2.0f * PlayerRadius,
                                0.4f + 0.2f * mt() / float(mt.max()));
    }
  }
...
void Game::send_state_message(Connection *connection_,
  ...
  auto send_player = [&](Player const &player) {
    ...
    connection.send(player.credit);

    // NOTE: can't just 'send(name)' because player.name is not plain-old-data
    // type. effectively: truncates player name to 255 chars
    uint8_t len = uint8_t(std::min<size_t>(255, player.name.size()));
    connection.send(len);
    connection.send_buffer.insert(connection.send_buffer.end(),
                                  player.name.begin(),
                                  player.name.begin() + len);
  };
...
bool Game::recv_state_message(Connection *connection_) {
  ...
  for (uint8_t i = 0; i < player_count; ++i) {
    ...
    uint8_t name_len;
    read(&name_len);
    // n.b. would probably be more efficient to directly copy from recv_buffer,
  ...
  read(&credit_point);
  std::cout << "Credit point: " << credit_point.x << ", " << credit_point.y
            << std::endl;
