...
#ifdef _WIN32
  { // when compiled on windows, check that code page is forced to utf-8 (makes
    // file loading/saving work right):
    // see:
    ...
    if (code_page == 65001) {
      ...
      std::cout
          << "WARNING: code page is set to " << code_page
          << " instead of 65001 (UTF-8). Some file handling functions may fail."
          << std::endl;
    }
  }
  ...
  try {
    ...
    while (true) {
      static auto next_tick = std::chrono::steady_clock::now() +
                              std::chrono::duration<double>(Game::Tick);
      ...
      while (true) {
        ...
        // helper used on client close (due to quit) and server close (due to
        // error):
        ...
        server.poll(
            [&](Connection *c, Connection::Event evt) {
