...
#define MSG_DONTWAIT                                                           \
  0 // on windows, sockets are set to non-blocking with an ioctl
typedef int ssize_t;
...
// NOTE: much of the sockets code herein is based on http-tweak's single-header
// http server
// see: https://github.com/ixchow/http-tweak

// Also, some help and examples for getaddrinfo from:
...
void poll_connections(
    char const *where, std::list<Connection> &connections,
    std::function<void(Connection *, Connection::Event event)> const &on_event,
    double timeout, Socket listen_socket = InvalidSocket) {
  ...
  { // wait (until timeout) for sockets' data to become available:
    ...
    // NOTE: on windows nfds is ignored --
    ...
    if (ret < 0) {
      std::cerr
          << "[" << where
          << "] Select returned an error; will attempt to read/write anyway."
          << std::endl;
    } else if (ret == 0) {
  ...
  if (listen_socket != InvalidSocket && FD_ISSET(listen_socket, &read_fds)) {
    ...
    if (got == InvalidSocket) {
...
#else
      {
        ...
        std::cerr << "[" << where << "] client connected on "
                  << connections.back().socket << "." << std::endl; // INFO
        if (on_event)
  ...
  for (auto &c : connections) {
    ...
    while (true) { // read until more data left to read
      ...
      if (ret < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
        ...
        if (ret == 0) {
          std::cerr << "[" << where << "] port closed, disconnecting."
                    << std::endl;
        } else if (ret < 0) {
          std::cerr << "[" << where << "] recv() returned error " << errno
                    << "(" << strerror(errno) << "), disconnecting."
                    << std::endl;
        } else {
          std::cerr
              << "[" << where
              << "] recv() returned strange number of bytes, disconnecting."
              << std::endl;
        }
        c.close();
        if (on_event)
  ...
  // process responses:
  for (auto &c : connections) {
    // don't bother with connections unless they are valid, have something to
    // send, and are marked writable:
    if (c.socket == InvalidSocket || c.send_buffer.empty() ||
        !FD_ISSET(c.socket, &write_fds))
      continue;

#ifdef _WIN32
    ssize_t ret =
        send(c.socket, reinterpret_cast<char const *>(c.send_buffer.data()),
             int(c.send_buffer.size()), MSG_DONTWAIT);
#else
    ssize_t ret =
        send(c.socket, reinterpret_cast<char const *>(c.send_buffer.data()),
             c.send_buffer.size(), MSG_DONTWAIT);
#endif
    if (ret < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
      ...
      if (ret < 0) {
        std::cerr << "[" << where << "] send() returned error " << errno
                  << ", disconnecting." << std::endl;
      } else {
        assert(ret == 0 || ret > (ssize_t)c.send_buffer.size());
        std::cerr << "[" << where
                  << "] send() returned strange number of bytes [" << ret
                  << " of " << c.send_buffer.size() << "], disconnecting."
                  << std::endl;
      }
      c.close();
      if (on_event)
  ...
  { // use getaddrinfo to look up how to bind to port:
    ...
    if (addrinfo_ret != 0) {
      throw std::runtime_error("getaddrinfo error: " +
                               std::string(gai_strerror(addrinfo_ret)));
    }
...
#ifdef _WIN32
        BOOL one = TRUE;
        int ret = setsockopt(s, SOL_SOCKET, SO_REUSEADDR,
...
void Server::poll(
    std::function<void(Connection *, Connection::Event event)> const &on_event,
    double timeout) {
  poll_connections("Server::poll", connections, on_event, timeout,
                   listen_socket);
...
Client::Client(std::string const &host, std::string const &port)
  ...
  { // use getaddrinfo to look up how to bind to host/port:
    ...
    std::cout << "[Client::Client] connecting to " << host << ":" << port << ":"
              << std::endl;
    // based on example code in the 'man getaddrinfo' man page on OSX:
    for (struct addrinfo *info = res; info != nullptr; info = info->ai_next) {
      { // DEBUG: dump info about this address:
    ...
    if (!connection) {
      throw std::runtime_error(
          "Failed to connect to any of the addresses tried for server.");
    }
  }
}
...
void Client::poll(
    std::function<void(Connection *, Connection::Event event)> const &on_event,
    double timeout) {
  poll_connections("Client::poll", connections, on_event, timeout,
                   InvalidSocket);
}
